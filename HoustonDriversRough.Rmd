...
p1 <- ggplot(NULL, aes(mu_sim_prior, tau_sim_prior)) +
  ...
  labs(xlab = 'Ratio', ylab = '', title = 'Sample Mean Prior: Normal')
...
theta_sim = as.matrix(posterior_sample)
x_sim = rgamma(nrow(theta_sim), theta_sim[,'lambda'])
# quantile(x_sim, c(0.025, 0.975))
hist(x_sim, freq = FALSE, xlab = "Ratio of raw speed to posted limit",
     main = "Posterior preditive distribution")
lines(density(x_sim))
...
# Drop levels or dataframe keeps the levels from superset
# So dumb this isn't automatic
s2$vehicle_make <- droplevels(s2$vehicle_make)
...
#   geom_boxplot() +
#   labs(title = 'Speed Distribution by Vehicle Make', xlab = 'Make', ylab = 'Ratio')
...
n_groups = nlevels(s2$vehicle_make)
...
posterior_sample <- coda.samples(model,
variable.names = c("mu", "mu0", "sigma_y", "sigma_mu"),
n.iter = 1000,
n.chains = 5)
summary(posterior_sample)$statistics[1:22, 1:2]
